...
*   实现代码_暴力尝试+记忆化搜索
    ...
    *   **且如果在该DP版本时如果发现DP表不好填写或者base case不好描述，那么就可以尝试换一个方向尝试**；因为大部分的DP都可以从多个角度和方向进行求解
...
*   解题思路
    ...
    *   如果我们只关心restA，那么restA和restB在递归的过程中总会有一个先为0，表示其中一个区域人数已满
    *   restA为0说明 剩下的所有司机只能去B区域；如果len(income) - now == restA说明当前位置及其往后所有司机都只能去A
    *   如果不是以上两种情况，那么说明当前司机可以在A和B中选择较大的策略

*   实现代码_递归尝试

    ```python
    income = [[10, 20], [25, 10], [5, 10], [5, 100]]
    # 判断数据状况是否合法 略
...
*   问题描述
    *   [问题地址](https://leetcode.cn/problems/21dk04/description/)

*   实现代码
    *   一眼动规，一眼行列对应模型，样本对应模型；**样本对应模型大部分以结尾组织可能性**
    *   两个指针，一个指向t结尾j，一个指向s结尾i
    *   如果当前两个指针指向的字符相同`s[i - 1] == t[j - 1]`，那么就有`p1 = DP[i - 1][j - 1]`；这种可能性是考虑s[i]在t的结尾,所以前提是它要存在
    *   p2就是不考虑s[i]在t中，那么`p2 = DP[i - 1][j]`
    *   `DP[i][j]`含义：s中取前j个字符，有多少个t中前i个字符组成的前缀串相等的子序列
...
<img src="images/image-20230214142435355.png" alt="image-20230214142435355" style="zoom:50%;" />
...
*   问题描述
    *   [问题地址](https://leetcode.cn/problems/minimum-cost-to-merge-stones/)

*   解题思路_DP

    *   通过观察和计算，一堆石头不可能k堆为一次合并，最终和并成一堆的公式(k != 2):`if len(stones) % (k - 1) != 1 and k != 2: return -1`

    *   化繁为简，如果定义一个递归函数，传入石头区间和目标堆数，返回最低消耗数
    *   如果传入的目标对数为1，那么就判断区间是否只有一个元素，如果只有一个元素返回0，不是一个元素就将区间分成k堆 + 合并k堆的消耗(这个可以通过预处理出的前缀和获得)
    *   如果区间中值不为1，且目标堆数不确定，就只能穷举，让前面一部分区间组成1堆，其他部分组成目标堆数 - 1；可以通过剪枝加速该过程
    *   优化，因为区间的开始和结束还有目标堆数的变化范围都是固定的；所以可以定义三维变量实现记忆化搜素
    ...
    *   ```python
        class Solution:
            def mergeStones(self, stones: List[int], k: int) -> int:
                ...
                DP = [[[None for j in range(len(stones))] for i in range(len(stones))] for z in range(k + 1)]
                ...
                def func(L, R, P):
                    ...
                    if L == R:
                        DP[P][L][R] =  0 if L == R else -1
                        return DP[P][L][R]
                    
                    if P == 1:
                        # 需要分为k分
                        if (R - L + 1) % (k - 1) != 1 and k != 2:
                            DP[P][L][R] = -1
                            return DP[P][L][R]
                        DP[P][L][R] = func(L, R, k) + preSum[R] - preSum[L] + stones[R]
                        return DP[P][L][R]
                    ...
                    for i in range(L, R):
                        ...
                        if p2 == -1:
                            continue
                        res = min(res, p1 + p2)
                    DP[P][L][R] = -1 if res == sys.maxsize else res
                    return DP[P][L][R]
                
                return func(0, len(stones) - 1, 1)
        ```
...
-   解题思路
    ...
    -   如果选择行列对应模型的话会发现，并不能通过缩小行列下标减小问题规模，因为有效括号一定是连续的
    -   这题考虑以每个位置结尾组织可能性；
    -   易得'('的位置一定是0；如果是')'那么就要找到与它匹配的'('，在尝试加上对应'('前一个位置的DP值
    -   找到自己对应的'(': 判断DP[i - 1]如果是0那么证明当前位置只能尝试和s[i - 1]结合；如果有值那么就通过偏移计算和当前i对应的'('
...
*   问题描述
    ...
    *   [问题地址](https://leetcode.cn/problems/wildcard-matching/)

*   解题思路

    *   行列对应模型，轻松拿下；优化可改滚动数组
...
*   问题描述
    *   [问题地址](https://leetcode.cn/problems/minimum-path-sum/description/)
*   解题思路
    *   对于每个单元格来说他们的初值都是自己原地的值，计算得到每个点到目标点的最小值最终返回最上角的值即可
    *   目标点的值为本身的值，普遍位置的值是`自身值+min(下侧单元格到目标点值, 右侧单元格到目标单点值)`
    ...
    -   ```python
        class Solution:
            def minPathSum(self, grid: List[List[int]]) -> int:
                ...
                for i in range(M - 1, -1, -1):
                    for j in range(N - 1, -1, -1):
                        p1 = DP[i + 1][j] if i + 1 < M else None
                        p2 = DP[i][j + 1] if j + 1 < N else None
                        if p1 == None or p2 == None:
                            if p1 == None and p2 == None:
                                continue
                            DP[i][j] += p1 if p1 != None else p2
                        else:
                            DP[i][j] += min(p1, p2)
...
*   问题描述
    ...
    *   [问题地址](https://leetcode.cn/problems/scramble-string/description/)

*   解题思路

    *   定义递归函数；功能：给定两个字符串(用4个下标表示)，判断是是否能扰乱得到s2
    *   base case：当两个字符串都只剩一个字符时，相等返回True否则False
    *   普遍情况: 尝试从s1的每个位置切开，p1：让s1的前半部分去和s2的后半部分(长度相同)匹配，p2：让s1的前半部分和s2等量的前半部分匹配`l:13-17`匹配成返回True；如果当前s1中每个位置切开都尝试匹配都失败就返回False`l:20`
    *   加上记忆化搜索拿下
    ...
    *   ```python
        class Solution:
            def isScramble(self, s1: str, s2: str) -> bool:
                memo = dict()
                def func(left, right, tLeft, tRight):
                    key = str(left) + '_' + str(right) + '_' + str(tLeft) + '_' + str(tRight)
                    if memo.get(key) != None:
                        return memo[key]
        
                    if left == right:
                        memo[key] = s1[left] == s2[tLeft]
                        return memo[key]
        
                    for cut in range(right - left):
                        if func(left, left + cut, tLeft, tLeft + cut) and func(left + cut + 1, right, tLeft + cut + 1, tRight)\
                        or\
                        func(left, left + cut, tRight - cut, tRight) and func(left + cut + 1, right, tLeft, tRight - cut - 1):
                            memo[key] = True
                            return memo[key]
                    memo[key] = False
                    return memo[key]
        
                return func(0, len(s1) - 1, 0, len(s2) - 1)
        ```
